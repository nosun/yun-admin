    ...
    <div class="container">
        <div id="change_pwd">
            ...
            <div class="login_cont">
                <form id="J_Form" action="<?php echo site_url()?>/system/change_site_settings" class="form-horizontal well" method="post" accept-charset="utf-8"> 
                    <div class="control-group">
                        <label class="control-label">站点名称：</label>
                        <div class="controls"><input class="input-normal control-text" type="text" name="site_name" value="<?php echo $settings->site_name; ?>"></div>
                    </div>
                    <div class="control-group">
                        <label class="control-label">网店链接：</label>
                        <div class="controls"><input class="input-normal control-text" type="text" name="shop_url" value="<?php echo $settings->shop_url; ?>" /></div>
                    </div>
                    <div class="control-group">
                        <label class="control-label">后台登录认证码：</label>
                        <div class="controls"><input class="input-normal control-text" type="text" name="auth_code" value="<?php echo $settings->auth_code; ?>"/></div>
                    </div>
                    <div class="control-group">
                        <label class="control-label">不开启操作时间点：</label>
                        <?php $not_oper_time = json_decode($settings->not_oper_time);?>
                        <?php for($i=0; $i<24; $i++):?>
                        <input type='checkbox' class='normal' name="not_oper_time[]" <?php echo in_array($i, $not_oper_time) ? "value='{$i}' checked='checked'" :"value='{$i}'"; ?> autocomplete="off" /><?php echo $i.'点';?>
                        <?php endfor;?>
                    </div>
                    <div class="control-group">
