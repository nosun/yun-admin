...
class Notice_model extends Yun_Model{
    ...
    function get_list($product_id=0,$category=0,$status=0,$start_date=0,$end_date=0,$limit=0,$start=0){
        
            $table= $this->db->dbprefix('notice');
            if($product_id){
            ...
            if($start_date){
                $this->db->where("$table.send_time >",$start_date);
            }
            if($end_date){
                $this->db->where("$table.send_time <",$end_date);
            }
            if ($limit)
            {
                $this->db->limit($limit);
    ...
    function get_num($product_id=0,$category=0,$status=0,$start_date=0,$end_date=0){
